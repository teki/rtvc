...
- [ ] Separate the TVC video path into CRTC timing, final TVC color/sync signal
  generation, and a television receiver that locks only to observed sync. See
  the [TVC Video Signal Pipeline and Television Sync plan](.agents/plans/tvc-video-signal-pipeline.md).
...
- [ ] Add a small Z80-aware C-like compiler with inline assembly and an
  integrated source/assembly view. See the
  [C80 Compiler and Integrated Source View plan](.agents/plans/c80-compiler.md).
  - Keep the language intentionally smaller and more predictable than ISO C.
  - Preserve source-to-assembly-to-address provenance for editor and debugger
    integration.
  - Show generated bytes and static T-state information without blocking
    real-time emulation.
